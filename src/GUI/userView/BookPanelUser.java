...
public class BookPanelUser extends JPanel implements ActionListener {
    ...
    public BookPanelUser(User user, Book book, UserFrame parentFrame) {
        ...
        switch (book.getState()) {
            case IN_ARCHIVE:
                ...
                List<LoanRequest> usersRequests = lrm.filterBy(LoanRequestsManager.Criteria.APPLICANT, user);
                boolean requestSent = false;
                for (LoanRequest r : usersRequests) {
                    if (r.getRequested().equals(book)) {
                        requestSent = true;
                        break;
                    }
                }
                ...
                if (requestSent) {
                    JLabel requestSentLabel = new JLabel("  Request sent  ");
                    requestSentLabel.setFont(new Font("Lexend", Font.ITALIC, 16));
                    requestSentLabel.setForeground(Color.GRAY);
                    buttonPanel.add(requestSentLabel);
                }
                else 
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == button) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, "The book is not in the archive", "Error",
